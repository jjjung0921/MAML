...
원문은 task를 네 요소의 묶음으로 정의한다. [fact] §2.1
    ...
    T = { L(x1, a1, ..., xH, aH),  q(x1),  q(x_{t+1} | x_t, a_t),  H }

- `L` — 그 task의 손실. 분류 오류일 수도, MDP의 비용일 수도 있다.
- `q(x1)` — 초기 관측의 분포.
- `q(x_{t+1} | x_t, a_t)` — 전이 분포 (RL용).
- `H` — episode 길이. **i.i.d. 지도학습에서는 H = 1이다.** [fact] §2.1

우리가 다루는 건 H = 1인 경우이므로, 전이 분포는 사실상 비어 있고 task는 "손실 + 데이터 분포"로 줄어든다.

그 위에 task들의 분포 `p(T)`가 있고, K-shot 설정에서 모델은 `p(T)`에서 뽑은 새 task를 **K개 샘플만으로** 배운다. [fact] §2.1

데이터가 두 번 나뉘는 지점이 Algorithm 2에 있다. [fact] `#eq-adapt` 및 Algorithm 2

    5:  D  = {x, y} K개를 T_i 에서 샘플     ← 적응(inner)에 쓴다
    7:  θ_i' = θ - α ∇_θ L_{T_i}(f_θ)        ← D 로 계산한 gradient
    8:  D' = {x, y} 를 T_i 에서 다시 샘플    ← meta-update 에 쓴다
    10: θ ← θ - β ∇_θ Σ_i L_{T_i}(f_{θ_i'})  ← D' 로 계산
...
1. K-shot에서 K를 5에서 50으로 늘리면 이 정식화의 어느 기호가 바뀌고, 어느 기호는 그대로인가?
2. "task를 훈련 예제로 본다"는 말이 성립하려면 `p(T)`에 최소한 어떤 조건이 필요할까? (원문이 명시하지 않은 것을 찾는 문제다)
