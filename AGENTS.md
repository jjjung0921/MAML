...
- Name: <학습 주제>
- Outcome: <이 학습이 끝나면 무엇을 할 수 있어야 하는지 한 줄>
- Learner: `LEARNER.md` (시간 예산 · 배경 · 입력 방식 · 파라미터)

<!-- 초기화(.ai/BOOTSTRAP.md) 시 채운다. 목표 상세는 LEARNING_GOALS.md에 둔다. -->
...
- 이 폴더 = subject 하나(`~/study/<subject>/`, concept map 하나를 공유하는 단위, DR-0005). 같은 폴더의 `why--<topic>.md` 같은 질문 원장은 스토리 측(`~/study/stories/`, 자체 AGENTS.md) 파일이며 concept state의 근거가 아니다 — 스토리의 개념이 목표와 이어지면 `concepts/`로 올린다. 다른 subject의 개념은 `requires`가 아니라 `evidence`로 그쪽 concept 파일을 가리키고, state는 그쪽이 기준이다
...
8. **State** — `concepts/` frontmatter의 `state`는 `exercises/` 기록으로만 바뀐다(`verified_by`가 가리키는 파일). introduced(학습자 언어의 Definition + Claims + Verification Criteria) → practiced(pass 또는 partial 1회) → verified(서로 다른 날 pass ≥ `verified_passes`, 그중 transfer·derivation·proof·coding ≥ 1). 튜터의 설명과 같은 세션의 되풀이는 근거가 아니다.
9. **Grading** — exercise 파일에 Rubric을 문제와 함께 먼저 쓴다(concept의 Verification Criteria에서 파생). 판정은 항목별 ✓/✗ + Attempt 인용이며 총평·칭찬은 쓰지 않는다. partial은 미통과, 두 등급 사이면 낮은 쪽, `grader`를 기록한다.
...
11. **Budget** — 세션은 선언된 분(`study-start.sh <분>` 또는 INBOX, 없으면 `LEARNER.md`의 `default_budget`) 안에서 끝낸다. 구성 순서: 복습 만기 → 진행 중 unit → 새 unit 하나 → 기록(마지막 5분). 예산이 `review_only_below` 미만이면 복습만. unit의 `est`·`actual`을 LOG에 적는다.
...
| Sandbox | `<실험 코드 실행 명령, 없으면 N/A>`      |

<!-- 초기화 시 Sandbox 줄을 채운다 (예: `uv run python sandbox/<file>.py`). -->

## Skills

<!-- 절차의 세부는 스킬에 위임한다. 계정에 없는 스킬은 건너뛴다. Codex를 쓰면 `.codex/skills/`에 같은 사본을 둔다. -->

| 상황 | 스킬 (있으면) |
|---|---|
| 목표를 "할 수 있다" 문장과 증명 방법으로 바꿀 때 | `problem-framing` |
| exercise 출제·채점 | `self-exam` |
...
- `<type>(<scope>): <summary>` — type: learn(concept·evidence), exercise, curriculum, docs, chore, wip. scope는 concept slug 또는 phase. 본문에 무엇을·왜. WIP는 미완료 사항.
...
- close commit: `docs(ai): close session — <요약>` (`.ai/`·`curriculum/`만 포함).
