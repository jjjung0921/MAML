...
하루 120분 안에서 짧은 세션으로 진행한다. unit마다 마지막 5분을 기록하며, 다음 진단부터 unit 160분 + 15개 unit의 기록 75분 + 중간 재개 기록 5분 = 총 240분이다. 휴식은 별도다.

| 날짜 | 학습 unit | 기록 | 산출물 |
|---|---|---|---|
| 1일차 · 09-05 | 진단·자료·운영 확인 30분 + Phase 02 U1~U4 45분 + U5 첫 5분 | 40분 | 과제와 목적함수 구분, meta-gradient 의존 경로 시작 |
| 2일차 · 09-06 | Phase 02 U5 나머지 10분 + U6~U9 40분 + Phase 03 30분 | 40분 | exact/FOMAML 비교, 후속 논문 독해 capstone |

unit이 시간 경계에 걸리면 Progress와 HANDOFF에 남기고 이어간다. 이틀은 학습과 초회 평가의 목표이며, 이해 상태 verified와 공식 Phase 완료는 서로 다른 날의 exercise 증거가 갖춰져야 한다. 재시도·추가 복습은 별도 시간이 필요할 수 있다.
...
원문을 가리키는 [자료 목록](evidence/INDEX.md)과 기존 분석을 함께 사용한다. 분석 문서가 있다는 사실은 학습자의 이해를 증명하지 않는다. 개념 Definition은 튜터가 초안을 쓰고 학습자가 검토하며(DR-0012, ai:draft 마킹), Attempt 전사 확인 후에만 채점한다.
